...
export class Lexer {
    ...
    while (this.pos < buf.length) {
      ...
      if (b === 0x5c) {
        ...
        switch (e) {
          ...
          case 0x5c: bytes.push(0x5c); break // backslash
          case 0x0d: if (buf[this.pos] === 0x0a) this.pos++; break // line continuation
  ...
  private readStreamBody(dict: PdfDict, resolveLength?: LengthResolver): PdfStream {
    ...
    let end = -1
    const declared = resolveLength ? resolveLength(dict.get('Length')) : asNumber(dict.get('Length'))
